...
const ChatIA = () => {
    ...
    const enviarPregunta = async (e) => {
        ...
        try {
            // Conectamos con el servidor usando la URL base centralizada
            const res = await axios.post(`${API_BASE_URL}/api/chat-ia`, {
                pregunta: miPregunta
            });
            ...
            setMensajes(prev => [...prev, {
                rol: 'ia',
                texto: '❌ ERROR DE CONEXIÓN: El núcleo del servidor no responde. Estamos fuera de frecuencia, hermano.',
                fecha: new Date().toLocaleTimeString()
            }]);
        } finally {
            setCargando(false);
