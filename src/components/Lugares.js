...
const Lugares = () => {
    ...
    const cargarDatos = useCallback(async () => {
        try {
            // CARGA TRIPLE: Lugares, Noticias e Imágenes de Usuarios
            // IMPORTANTE: Asegúrate de que las rutas del server coincidan con estas 3
            ...
            // Filtramos por estado para que el mapa no sea un caos
        ...
        <section className="panel-admin-container fade-in">
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px', gap: '15px', flexWrap: 'wrap' }}>
                <button 
                    ...
                    style={{ 
                        background: vista === 'lugares' ? '#00ff41' : '#222', 
                        color: vista === 'lugares' ? '#000' : '#00ff41', 
                        border: '2px solid #00ff41', padding: '10px 15px', fontWeight: 'bold', cursor: 'pointer'
                    }}
                > 📍 LUGARES </button>
                ...
                <button 
                    ...
                    style={{ 
                        ...
                        border: '2px solid #ff4444', padding: '10px 15px', fontWeight: 'bold', cursor: 'pointer'
                    }}
                > ⚠️ ALERTAS </button>
                ...
                <button 
                    ...
                    style={{ 
                        background: vista === 'archivos' ? '#00d4ff' : '#222', 
                        color: vista === 'archivos' ? '#000' : '#00d4ff', 
                        border: '2px solid #00d4ff', padding: '10px 15px', fontWeight: 'bold', cursor: 'pointer'
                    }}
                ...
                <div className="mapa-mando">
                    <MapContainer center={centroMapa} zoom={13} style={{ height: '100%', width: '100%' }}>
                        ...
                        {vista === 'lugares' && puntos.map(l => (
                            <Marker key={`l-${l.id}`} position={[l.latitud, l.longitud]} icon={idResaltado === l.id ? iconoResaltado : iconoLugar}>
                                <Popup>
                                    <div className="popup-bunker">
                                        <h4 style={{ color: '#00ff41' }}>📍 {l.nombre}</h4>
                                        {l.imagen_url && <img src={`http://localhost:5000/lugares/${l.imagen_url}`} alt={l.nombre} style={{ width: '100%', borderRadius: '4px' }} />}
                        ...
                        {/* RENDER DE ALERTAS */}
                        {vista === 'noticias' && noticias.map(n => (
                            <Marker key={`n-${n.id}`} position={[n.latitud, n.longitud]} icon={idResaltado === n.id ? iconoResaltado : iconoNoticia}>
                                <Popup>
                                    <div className="popup-bunker">
                                        <h4 style={{ color: '#ff4444' }}>⚠️ ALERTA</h4>
                                        <strong style={{ color: '#00ff88' }}>{n.titulo}</strong>
                                        {n.imagen_url && <img src={`http://localhost:5000/imagenes/${n.imagen_url}`} alt={n.titulo} style={{ width: '100%', borderRadius: '4px' }} />}
                                        <p style={{ fontSize: '11px' }}>{n.cuerpo}</p>
                                    </div>
                                </Popup>
                            </Marker>
                        ))}

                        {/* RENDER DE ARCHIVOS USUARIOS */}
                        {vista === 'archivos' && archivos.map(a => (
                            <Marker key={`a-${a.id}`} position={[a.latitud, a.longitud]} icon={idResaltado === a.id ? iconoResaltado : iconoArchivo}>
                                <Popup>
                                    <div className="popup-bunker">
                                        <h4 style={{ color: '#00d4ff' }}>📂 ARCHIVO DE AGENTE</h4>
                                        <img 
                                            src={`http://localhost:5000/archivos-usuarios/${a.url_imagen}`} 
                                            alt={a.titulo} 
                        ...
                        {(vista === 'lugares' ? puntos : vista === 'noticias' ? noticias : archivos).map(item => (
                            <div 
                                ...
                                style={{
                                    ...
                                    background: idResaltado === item.id ? 'rgba(255, 255, 255, 0.05)' : 'transparent',
                            ...
                            >
                                ...
                                <div style={{ fontSize: '0.75rem', color: '#666' }}>
                                    {vista === 'archivos' ? `Por: ${item.agente}` : item.ubicacion || 'Sector Alhambra'}
                                </div>
                            </div>
                        ))}
                    </div>
