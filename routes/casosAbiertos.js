...
module.exports = (upload) => {
    ...
    try {
        const query = `
            ...
            (titulo, contenido, titulo_en, contenido_en, latitud, longitud, imagen_url, estado, youtube_url) 
            VALUES (?, ?, ?, ?, ?, ?, ?, 'pendiente', ?)
        `;
    ...
    router.put('/:id', upload ? upload.single('imagen') : (req, res, next) => next(), async (req, res) => {
        const { id } = req.params;
    const { titulo, contenido, titulo_en, contenido_en, latitud, longitud, ubicacion, url_externa, fuente_url, youtube_url, capturas, imagen_posicion } = req.body;
    
    // Check if there is an image uploaded
    const nombreArchivo = req.file ? (req.file.path || req.file.filename) : null;
    
    try {
        let query = "UPDATE casos_abiertos SET titulo = ?, contenido = ?, titulo_en = ?, contenido_en = ?, latitud = ?, longitud = ?, fuente_url = ?, youtube_url = ?, capturas = ?, imagen_posicion = ?";
        let valores = [titulo, contenido, titulo_en || null, contenido_en || null, latitud || 0, longitud || 0, fuente_url || url_externa || null, youtube_url || null, capturas || null, imagen_posicion || null];
