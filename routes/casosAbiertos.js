...
module.exports = (upload) => {
    ...
    router.put('/:id', upload ? upload.single('imagen') : (req, res, next) => next(), async (req, res) => {
        const { id } = req.params;
    const { titulo, contenido, titulo_en, contenido_en, latitud, longitud, ubicacion, url_externa, fuente_url, youtube_url, capturas } = req.body;
    
    // Check if there is an image uploaded
    const nombreArchivo = req.file ? (req.file.path || req.file.filename) : null;
    
    try {
        let query = "UPDATE casos_abiertos SET titulo = ?, contenido = ?, titulo_en = ?, contenido_en = ?, latitud = ?, longitud = ?, fuente_url = ?, youtube_url = ?, capturas = ?";
