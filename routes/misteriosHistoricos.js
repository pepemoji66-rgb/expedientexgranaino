...
module.exports = (upload) => {
    ...
    router.put('/:id', upload ? upload.single('imagen') : (req, res, next) => next(), async (req, res) => {
        const { id } = req.params;
        const { titulo, contenido, titulo_en, contenido_en, latitud, longitud, fuente_url, url_externa, youtube_url, capturas, imagen_posicion } = req.body;
        const nombreArchivo = req.file ? (req.file.path || req.file.filename) : null;
        
        try {
            let query = "UPDATE misterios_historicos SET titulo = ?, contenido = ?, titulo_en = ?, contenido_en = ?, latitud = ?, longitud = ?, fuente_url = ?, youtube_url = ?, capturas = ?, imagen_posicion = ?";
