...
module.exports = (db, upload) => {
    ...
    router.put('/:id', upload.single('imagen'), async (req, res) => {
        const { titulo, contenido, latitud, longitud, estado, tipo, fuente_url, url_externa, youtube_url, capturas, imagen_posicion } = req.body;
        const url_final = fuente_url || url_externa || null;
        const imagen_url = req.file ? (req.file.path || req.file.filename) : null;
        try {
            if (imagen_url) {
                await db.execute(
                    "UPDATE expedientes SET titulo = ?, contenido = ?, latitud = ?, longitud = ?, estado = ?, tipo = ?, imagen_url = ?, fuente_url = ?, youtube_url = ?, capturas = ?, imagen_posicion = ? WHERE id = ?",
                    [titulo, contenido, latitud, longitud, estado, tipo, imagen_url, url_final, youtube_url || null, capturas || null, imagen_posicion || null, req.params.id]
                );
                ...
                await db.execute(
                    "UPDATE expedientes SET titulo = ?, contenido = ?, latitud = ?, longitud = ?, estado = ?, tipo = ?, fuente_url = ?, youtube_url = ?, capturas = ?, imagen_posicion = ? WHERE id = ?",
                    [titulo, contenido, latitud, longitud, estado, tipo, url_final, youtube_url || null, capturas || null, imagen_posicion || null, req.params.id]
                );
            }
